...
{
    __global_warnings_as_errors = enabled;
}

void set_suppress_warnings(bool enabled)
{
    __global_suppress_warnings = enabled;
}
...
void print_diagnostics(const Error* error)
{
    ...
    fprintf(stderr, "%s:%zu:%zu:%s%s%s: %s\n", error->file, error->line, error->column, color, severity_to_string(error->severity), reset, error->message);
}
...
ErrorList* append_error(ErrorList* error_list, const char* file, const char* message, uint32_t line, uint32_t column, Severity severity, Category category)
{
    ...
    {
        uint32_t new_capacity = error_list->capacity * 2;
    ...
    Error error = {message, file, line, column, (uint32_t)strlen(message), severity, category};
...
void warn(ErrorList* error_list, const char* file, uint32_t line, uint32_t column, Category category, const char* format, ...)
{
    ...
    if (__global_suppress_warnings && !__global_warnings_as_errors)
    {
        return;
    }
...
{
    if (!__global_verbose_mode || !error_list || !format)
    {
        return;
    }
