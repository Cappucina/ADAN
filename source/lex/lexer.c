...
static TokenType is_keyword(const char* name, uint32_t length)
{
...
Token lex(Lexer* lx)
{
    ...
    {
        error(lx->errors, "source", lx->line, lx->column, LEXER, "Unexpected character '%c'", c);
        next_char(lx);
