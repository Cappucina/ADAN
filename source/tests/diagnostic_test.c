...
static int test_append_error(void)
{
    ErrorList* list = create_errors();
    ASSERT_NOT_NULL(list, "\"create_errors\" should not return NULL");

    ErrorList* result = append_error(list, "main.adn", "Test error", 1, 5, ERROR, LEXER);
    ASSERT_NOT_NULL(result, "\"append_error\" should return non-NULL");
...
static int test_multiple_errors(void)
{
    ...
    append_error(list, "main.adn", "Error 1", 1, 0, ERROR, LEXER);
    append_error(list, "main.adn", "Error 2", 2, 0, WARNING, PARSER);
    append_error(list, "main.adn", "Error 3", 3, 0, CRITICAL, SEMANTIC);
...
static int test_error_message(void)
{
    ...
    append_error(list, "main.adn", msg, 5, 10, ERROR, CODEGEN);
...
static int test_error_location(void)
{
    ...
    append_error(list, "main.adn", "Error at location", 42, 15, ERROR, GENERIC);
...
int run_diagnostic_tests(TestSuite* suite)
{
    ...
    test_suite_run_test(suite, "append_error", test_append_error);
