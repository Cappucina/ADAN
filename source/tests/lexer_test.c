...
// static const char* token_to_string(Tokens type)
// {
//     switch (type)
//     {
//         case TOKEN_EOF:
...
//         case TOKEN_ADD_ADD:
//             return "TOKEN_ADD_ADD";
//         case TOKEN_SUB_SUB:
//             return "TOKEN_SUB_SUB";
...
static int test_lex_operator_type_declarator(void)
{
    ...
    ASSERT_EQ(tok.type, TOKEN_TYPE_DECL, "token should be type declarator operator");
...
static int test_lex_single_token_number(void)
{
    ...
    Token tok = lex(lexer);
    if (tok.type != TOKEN_INT_LITERAL && tok.type != TOKEN_INT)
    {
        printf("Expected TOKEN_INT_LITERAL or TOKEN_INT, got %d\n", tok.type);
        return 1;
    }
...
static int test_lex_single_token_paren(void)
{
    ...
    Token tok = lex(lexer);
    ASSERT_EQ(tok.type, TOKEN_OPEN_PAREN, "lex should recognize left paren");
...
static int test_lex_single_token_brace(void)
{
    ...
    Token tok = lex(lexer);
    ASSERT_EQ(tok.type, TOKEN_OPEN_BRACE, "lex should recognize left brace");
...
static int test_lex_single_token_bracket(void)
{
    ...
    Token tok = lex(lexer);
    ASSERT_EQ(tok.type, TOKEN_OPEN_BRACKET, "lex should recognize left bracket");
...
static int test_lex_example_program(void)
{
    ...
    if (!f)
    {
        printf("Skipping example_program test: examples/adan.adn not found\n");
        return 0;
    }
