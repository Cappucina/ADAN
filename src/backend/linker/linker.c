									 ...
									 const char* bundle_csv)
{
	...
	while (tok)
	{
		...
		if (path[0] == '\0') { tok = strtok_r(NULL, ",", &saveptr); continue; }
		...
		if (stat(path, &st) == 0 && S_ISDIR(st.st_mode))
		{
			...
			while ((e = readdir(d)) != NULL)
			{
				...
				{
					char srcpath[4096];
					snprintf(srcpath, sizeof(srcpath), "%s/%s", path, e->d_name);
					char tmpobj[4096];
					snprintf(tmpobj, sizeof(tmpobj), "/tmp/adan_bundle_%d_%d.o", (int)pid, tmpid++);
					size_t cmdlen = strlen(srcpath) + strlen(tmpobj) + 64;
					char* cmd = malloc(cmdlen);
					if (!cmd) { closedir(d); goto cleanup_error; }
					snprintf(cmd, cmdlen, "clang -c %s -o %s", srcpath, tmpobj);
					int r = run_cmd(cmd);
					free(cmd);
					if (r != 0) { closedir(d); goto cleanup_error; }
